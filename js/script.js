...
function displayTime(time) { //get current time and duration  function
	let getMinutes = time / 60;
	let hours = (getMinutes / 60);
...
function volumeDrag(e){
 ...
 function volumeMoveHandler(e) {
	...
	if(video.volume == 0){
		...
		mute_unmute_btn.innerHTML = `<svg height="100%" version="1.1" viewBox="0 0 36 36" width="100%"><defs><clipPath ><path d="m 14.35,-0.14 -5.86,5.86 20.73,20.78 5.86,-5.91 z"></path><path d="M 7.07,6.87 -1.11,15.33 19.61,36.11 27.80,27.60 z"></path><path  d="M 9.09,5.20 6.47,7.88 26.82,28.77 29.66,25.99 z" transform="translate(0, 0)"></path></clipPath><clipPath ><path d="m -11.45,-15.55 -4.44,4.51 20.45,20.94 4.55,-4.66 z" transform="translate(0, 0)"></path></clipPath></defs><path  d="M8,21 L12,21 L17,26 L17,10 L12,15 L8,15 L8,21 Z M19,14 L19,22 C20.48,21.32 21.5,19.77 21.5,18 C21.5,16.26 20.48,14.74 19,14 Z" fill="#fff" ></path><path  d="M 9.25,9 7.98,10.27 24.71,27 l 1.27,-1.27 Z" fill="#fff"  style="display: none;"></path></svg>`;
...
// show controls with onmouseenter

function showControlsEventsFunc(){
	if(video.paused == false){
		showControlsStyle();
		
	}
}
// hide controls with onmouseleave
function hideControlsEventsFunc(){
	if(video.paused){
		showControlsStyle();
	}else{
		hideControlsStyle();
...
function startInactivityTimer(){
	inactivityTimer = window.setTimeout(doInactivity,delayTime);
}
...
function localStorageVolumeFunc(){
...
// display time 
function displayCurrentandDuration(){
	current_time.textContent = `${displayTime(video.currentTime)}`;
	duration_time.textContent = `${displayTime(video.duration)}`;
}
// buffering function 
function bufferingTrack(){
	    let percent = null;
    // FF4+, Chrome
...
// event of buffering track
video.onprogress = bufferingTrack;
// events of display current time and duration
video.onloadedmetadata = displayCurrentandDuration;


if(video.duration > 0){
	displayCurrentandDuration();
}
localStorageVolumeFunc();
}// end init_player


window.addEventListener("DOMContentLoaded",init_player);
// all callback events
